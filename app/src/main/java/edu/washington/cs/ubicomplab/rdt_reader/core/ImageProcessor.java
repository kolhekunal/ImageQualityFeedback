...
import static org.opencv.imgproc.Imgproc.LaplacianImage;
...
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.ANGLE_THRESHOLD;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.BLOOD_COLOR_HIGH_HUE_LOWER;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.BLOOD_COLOR_HIGH_HUE_UPPER;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.BLOOD_COLOR_LOW_HUE_LOWER;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.BLOOD_COLOR_LOW_HUE_UPPER;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.BLOOD_PERCENTAGE_THRESHOLD;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.CLAHE_CLIP_LIMIT;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.CLAHE_WIDTH;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.CONTROL_LINE_NAME;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.CROP_RATIO;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.FIDUCIAL_SEARCH_NUM_CLUSTERS;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.GLARE_WHITE_RATIO;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.GLARE_WHITE_VALUE;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.GOOD_MATCH_COUNT;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.MOVE_CLOSER_COUNT;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.OVER_EXPOSURE_THRESHOLD;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.OVER_EXPOSURE_WHITE_COUNT;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.POSITION_THRESHOLD;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.RANSAC;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.RESULT_WINDOW_ENHANCE_THRESHOLD;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.SHARPNESS_THRESHOLD;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.SIFT_RESIZE_FACTOR;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.SIZE_THRESHOLD;
import static edu.washington.cs.ubicomplab.rdt_reader.core.Constants.UNDER_EXPOSURE_THRESHOLD;
...
public class ImageProcessor {
    ...
    private MatOfPoint2f detectRDT(Mat inputMat) {
        ...
        resize(inputMat, scaledMat, new Size(), scale, scale, Imgproc.INTER_LINEAR);
        ...
        if (goodMatches.size() > GOOD_MATCH_COUNT) {
            ...
            Mat H = Calib3d.findHomographyImage(objMat, sceneMat, Calib3d.RANSAC, RANSAC);
    ...
    private double measureSharpness(Mat inputMat) {
        ...
        Mat des = new Mat();
        LaplacianImage(inputMat, des, CvType.CV_64F);
    ...
    private boolean checkSharpness(Mat inputMat) {
        ...
        resize(inputMat, resized,
                new Size(inputMat.size().width*scale,inputMat.size().height*scale),0.0,0.0,0);
    ...
    private Mat cropResultWindow(Mat inputMat, MatOfPoint2f boundary, int offset) {
        ...
        if (correctedMat.width() > 0 && correctedMat.height() > 0)
            resize(correctedMat, correctedMat,
                    new Size(mRDT.resultWindowRect.width, mRDT.resultWindowRect.height),0.0,0.0,0);
        return correctedMat;
    }
    ...
    private Mat correctPerspective(Mat inputMat, MatOfPoint2f boundary) {
        ...
        warpPerspective(inputMat, correctedMat, M, new Size(mRDT.refImg.cols(), mRDT.refImg.rows()),0);
    ...
    private void opencvProcess(int refImageID) {
        ...
        Imgproc.LaplacianImage(matImageGrey, laplacianImage, CvType.CV_8U);
